...
const injectionPatterns: InjectionPattern[] = [
  {
    ...
    category: 'direct',
    pattern:
      /ignore\s+(all\s+)?(previous|prior|above)\s+(instructions?|directives?|commands?|prompts?)/i,
    severity: 'high',
  ...
  },
  {
    ...
    category: 'direct',
    pattern: /(here\s+are|follow|obey)\s+(my\s+)?new\s+(instructions?|rules?|directives?)/i,
    severity: 'high',
  ...
  },
  {
    ...
    category: 'direct',
    pattern:
      /(reveal|show|display|print|output|tell\s+me)\s+(your\s+)?(system\s+)?(prompt|instructions?|directives?)/i,
    severity: 'high',
  ...
  },
  {
    name: 'act-as',
    category: 'direct',
    pattern: /\bact\s+as\s+(if\s+you\s+are\s+)?(a\s+|an\s+)?/i,
    severity: 'medium',
  ...
  },
  {
    ...
    category: 'direct',
    pattern: /forget\s+(everything|all\s+previous|your\s+training)/i,
    severity: 'high',
  ...
  },
  {
    ...
    category: 'direct',
    pattern: /imagine\s+(you\s+are|yourself\s+as)\b/i,
    severity: 'medium',
    description: 'Imagination-based role injection',
  },

  {
    name: 'url-payload-injection',
    category: 'indirect',
    pattern: /https?:\/\/\S*(?:prompt|inject|override|command|system)\S*/i,
    severity: 'medium',
    description: 'URL-based payload injection',
  },
  {
    name: 'markdown-image-exploit',
    category: 'indirect',
    pattern: /!\[.*?\]\(\s*(?:https?:\/\/|data:).*?(?:prompt|inject|instruction)/i,
    severity: 'medium',
    description: 'Markdown image with injection payload',
  },
  {
    name: 'data-exfiltration',
    category: 'indirect',
    pattern:
      /(?:send|forward|copy|paste|exfiltrate)\s+.*?\b(?:to\s+)?(?:https?:\/\/|webhook|endpoint|server)/i,
    severity: 'high',
  ...
  },
  {
    name: 'url-encoding',
    category: 'encoding',
    pattern: /%[0-9A-Fa-f]{2}(?:%[0-9A-Fa-f]{2})+/g,
    severity: 'low',
  ...
  },
  {
    name: 'role-switching',
    category: 'boundary',
    pattern: /(?:switch|change)\s+(?:role|mode|persona|context)\s+(?:to|into)\b/i,
    severity: 'medium',
  ...
  },
  {
    ...
    category: 'boundary',
    pattern: /<\/?(?:system|instruction|prompt|rule|directive)>/i,
    severity: 'medium',
...

  {
    ...
    pattern:
      /\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b.*?\b(?:send|output|print|echo)\b/i,
    severity: 'high',
  ...
  },
  {
    ...
    category: 'exfiltration',
    pattern: /(?:discord|slack|telegram|webhook)\..*?(?:api|hook|send)/i,
    severity: 'high',
...

  {
    ...
    category: 'direct',
    pattern: /^(?:DAN|Developer\s+Mode|Jailbreak|Unrestricted)\b/im,
    severity: 'high',
  ...
  },
  {
    ...
    category: 'direct',
    pattern:
      /(?:do\s+not\s+(?:ask|confirm|verify|check)|without\s+(?:asking|confirming|verification))/i,
    severity: 'high',
    description: 'Confirmation bypass attempt',
  },
  {
    name: 'output-override',
    category: 'direct',
    pattern:
      /(?:start\s+your\s+response\s+with|your\s+output\s+must|you\s+must\s+output|respond\s+with\s+exactly)/i,
    severity: 'high',
  ...
  },
  {
    ...
    category: 'direct',
    pattern: /(?:hidden|secret|covert)\s+(?:instruction|prompt|command|rule)/i,
    severity: 'high',
  ...
  },
  {
    ...
    category: 'direct',
    pattern:
      /(?:bypass|circumvent|get\s+around|work\s+around)\s+(?:the\s+)?(?:filter|safety|guard|restriction|rule)/i,
    severity: 'high',
  ...
  },
  {
    ...
    category: 'direct',
    pattern:
      /(?:split\s+(?:this|the|your)\s+(?:response|output|message)|respond\s+in\s+(?:parts|pieces|segments))/i,
    severity: 'medium',
  ...
  },
  {
    ...
    category: 'boundary',
    pattern: /\[\[(?:system|override|inject|instruction|command)\s*[:=]\s*.*?\]\]/i,
    severity: 'high',
    description: 'Meta-instruction bracket injection',
  },
  {
    name: 'crescendo-attack',
    category: 'direct',
    pattern:
      /(?:let's|let\s+us)\s+(?:play\s+a\s+game|do\s+a\s+roleplay|have\s+a\s+conversation\s+where)/i,
    severity: 'medium',
    description: 'Crescendo-style social engineering',
  },
  {
    ...
    pattern: /[\u0400-\u04FF\u2000-\u206F\uFF00-\uFFEF]{2,}/g,
    severity: 'low',
    description: 'Unicode homoglyph content',
  },
  {
    name: 'zero-width',
    category: 'encoding',
    pattern: /[\u200B-\u200F\u2028\u2029\uFEFF]/g,
    severity: 'low',
    description: 'Zero-width character content',
  },
];
...
function scanText(
...
): { detected: InjectionPattern[]; score: number } {
  ...
  for (const ip of injectionPatterns) {
    ...
    if (contextFilter !== 'all' && ip.category === 'indirect' && contextFilter === 'user_input') {
      continue;
    }
      ...
      const output = JSON.stringify(
        {
          ...
          detectedPatterns: detected.map((d) => ({
            name: d.name,
            category: d.category,
            severity: d.severity,
  ...
  execute: async (args: Record<string, unknown>, _ctx: ToolContext): Promise<ToolCallResult> => {
    ...
    try {
      ...
      return {
        ...
        success: false,
        output: '',
        error: `List patterns failed: ${error instanceof Error ? error.message : String(error)}`,
        durationMs: Date.now() - start,
      };
    }
  },
    ...
    try {
      ...
      if (action === 'list') {
        return {
          ...
          output: JSON.stringify(
            { whitelisted: Array.from(whitelistedPatterns), count: whitelistedPatterns.size },
            null,
            2,
          ),
          durationMs: Date.now() - start,
      ...
      if (!pattern) {
        return {
          ...
          success: false,
          output: '',
          error: 'pattern is required for add/remove actions',
          durationMs: Date.now() - start,
        };
      }
      ...
      if (action === 'add') {
        ...
        return {
          toolName: 'injection_whitelist',
          success: true,
          output: `Added "${pattern}" to whitelist`,
          durationMs: Date.now() - start,
        };
      }
      ...
      if (action === 'remove') {
        ...
        return {
          ...
          success: true,
          output: removed
            ? `Removed "${pattern}" from whitelist`
            : `Pattern "${pattern}" not in whitelist`,
          durationMs: Date.now() - start,
        };
      }
      ...
      return {
        ...
        success: false,
        output: '',
        error: `Whitelist operation failed: ${
      ...
      const output = JSON.stringify(
        {
          ...
          blockRate: detectionStats.total > 0
            ? `${Math.round((detectionStats.blocked / detectionStats.total) * 100)}%`
            : '0%',
